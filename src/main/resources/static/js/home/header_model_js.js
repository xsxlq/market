...
//     });
    ...
    $('.nav_search_input').on("focus", function () {
        $(this).animate({width: "150px", marginLeft: "20%"}, 450);
    });

    $('.nav_search_input').on("blur", function () {
        if ($(this).val() == '')
            $(this).animate({width: "104px", marginLeft: "25%"}, 450);
    });
    ...
    $('.login_or_register_string').hover(function () {
            $('.personal_nav').slideDown(0)
                .animate({height: '400'});
    },function(){
        $(".personal_nav").slideUp(0)
            .animate({height: '0'});
    })
